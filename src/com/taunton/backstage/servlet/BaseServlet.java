...
public class BaseServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request,HttpServletResponse response)
		...
		methodName = GetReqParamUtils.getParameter(request, "methodName", null, null);
		Method method = null;
		try {
			method = this.getClass().getMethod(methodName,HttpServletRequest.class, HttpServletResponse.class);
		} catch (Exception e1){
			throw new RuntimeException("后台管理系统通过方法名: "+methodName+"获取Method对象失败"+e1);
		}
//		if(methodName!=null){	
			String result = null;
			...
			try {
				...
				logger.info("后台管理系统执行"+methodName+"方法");
