 ...
 * 
 */
public class SendChangePayPwdSMS extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static Logger logger = Logger.getLogger(SendChangePayPwdSMS.class); 
...
//		String operate = GetReqParamUtils.getParameter(request, "operate", null, null);
		...
		logger.info("IP = "+reqAddr+" 发送修改支付密码验证码");
		...
		req.setSmsParamString("{code:'"+fixLenthString+"',operate:'修改支付密码'}");
		req.setRecNum(tel);
		//在阿里大于服务端配置的短信模板id
		req.setSmsTemplateCode("SMS_60020079");
		AlibabaAliqinFcSmsNumSendResponse rsp = null;
		logger.info("发送手机修改支付密码验证码请求报文:"+req.getTextParams().toString());
		try {
			rsp = client.execute(req);
		} catch (ApiException e) {
			e.printStackTrace();
		}
		logger.info("接收手机修改支付密码验证码响应报文:"+rsp.getBody());
		String subCode = rsp.getSubCode();
		if (subCode == null) {
			String changePayPwdCodeTime = TimeUtils.formatDate(new Date(),TimeUtils.FORMATTER_1);
			
			
			request.getSession().setAttribute("changePayPwdTel", tel);
			request.getSession().setAttribute("changePayPwdCode", fixLenthString);
			request.getSession().setAttribute("changePayPwdCodeTime", changePayPwdCodeTime);
