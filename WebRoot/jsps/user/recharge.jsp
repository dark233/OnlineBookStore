...
<html>
  <head>
	...
	-->
	<link rel="stylesheet" type="text/css" href="<c:url value='/jsps/css/user/recharge.css'/>">
	...
	<script type="text/javascript">
		...
		var nextStep = function(){
			var $rechargeVal = $("#rechargeVal");
			var rechargeVal = $rechargeVal.val();
			rechargeVal = $.trim(rechargeVal);
			if(!rechargeVal){
				$("#rechargeVal~span").remove();
				$rechargeVal.after("<span style=\"color:#c30\">充值金额不能为空</span>");
				return false;
			}
			if(isNaN(rechargeVal)){
				$("#rechargeVal~span").remove();
				$rechargeVal.after("<span style=\"color:#c30\">充值金额必须为合法数字</span>");
				return false;
			}
			$("#form1").submit();
		};
  ...
  <body>
	...
	<form action="<c:url value='/UserServlet'/>" method="post" id="form1" target="_top">
		<input type="hidden" name="methodName" value="recharge"/>
		<div class="divPayway">
			...
			<div style="margin-left: 20px;">
					<input id="rechargeVal" type="text" name="rechargeVal" placeholder="输入充值金额" required="required"/>
			</div>
		</div>
