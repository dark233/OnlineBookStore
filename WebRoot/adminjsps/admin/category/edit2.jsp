...
<html>
  <head>
	...
	<script type="text/javascript">
		function checkForm() {
			if(!$("#cname").val()) {
				qikoo.dialog.alert("分类名不能为空！");
				return false;
			}
			if(!$("#pid").val()) {
				qikoo.dialog.alert("一级分类不能为空！");
				return false;
			}
			if(!$("#desc").val()) {
				qikoo.dialog.alert("分类描述不能为空！");
				return false;
			}
			return true;
		}
...
<style type="text/css">
  ...
  <body>
    ...
    <form action="<c:url value='/backstage/CategoryServlet'/>" method="post" onsubmit="return checkForm()">
    	<input type="hidden" name="methodName" value="editChild"/>
